...
import CallBackRequestDetails from "./CallBackRequestDetails/CallBackRequestDetails";
...
class CallBackRequest extends React.Component {
  state = {
    columns: [
      {
  ...
  handleGetApi = () => {
    API.get("/get_all_queries")
      .then((response) => {
        this.setState({ data: response.data?.filter(x => x.flag === "request_for_call") });
      })
      .catch((err) =>
  ...
  render() {
    ...
    return (
      <Card>
        <CardHeader>
          <CardTitle>Call Back Request List</CardTitle>
        </CardHeader>
